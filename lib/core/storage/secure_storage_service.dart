...
class SecureStorageService {
  ...
  /// Persisted light/dark theme choice. Returns null when nothing has
  /// been saved yet (new install, or before the user ever opens
  /// Settings) — callers should treat that as light/default.
  Future<bool?> readIsDarkMode() async {
    final raw = await _storage.read(key: AppConstants.themeModeStorageKey);
    if (raw == null) return null;
    return raw == 'dark';
  }

  Future<void> saveIsDarkMode(bool isDark) => _storage.write(
