...
class ResponsiveShell extends ConsumerWidget {
  ...
  /// Shown under the "Kezera" wordmark, e.g. "Employer Backoffice".
  ...
  /// picker. Also rendered on wide screens (in a slim bar above the
  /// content, since the sidebar layout has no AppBar of its own) and in
  /// the [forceSidebar] drawer layout's AppBar, so these show up
  /// consistently regardless of screen size or nav style.
  ...
  Widget build(BuildContext context, WidgetRef ref) {
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  // Wide screens have no AppBar of their own — the sidebar
                  // covers branding/nav — but [headerActions] (e.g. the
                  // notification bell) still needs a home here, or it
                  // silently never renders on desktop/web for any role
                  // that passes it. Only takes up space when there's
    ...
    if (forceSidebar) {
      return Scaffold(
        // `actions` was missing here, so any role using `forceSidebar`
        // (admin, agency) silently never showed its [headerActions] on
        // narrow screens either — the drawer swallowed the nav, but the
        // notification bell/language switcher had nowhere to render.
        appBar: AppBar(title: Text(brandLabel), actions: headerActions),
        drawer: Drawer(
          child: SafeArea(
            child: _Sidebar(
              brandLabel: brandLabel,
