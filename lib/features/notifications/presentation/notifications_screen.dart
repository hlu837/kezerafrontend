  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return InkWell(
      onTap: () {
        ...
        // "New message" notifications carry `placementId` in `data` (see
        // `notifications.service.js#notifyNewMessage`) — jump straight
        // into that conversation instead of just marking it read in
        // place. No candidate/job name comes along with the
        // notification, so `PlacementChatScreen` falls back to a
        // generic title; pushed on the root navigator (not this
        // screen's own nested one) so it opens as a normal full-screen
        // page rather than nesting inside whatever shell chrome is
        // currently on screen — see `PlacementChatScreen`'s doc comment
        // on why every entry point does this.
        if (notification.type == NotificationType.newMessage) {
          final placementId = notification.data['placementId'] as String?;
          if (placementId == null) return;
          Navigator.of(context, rootNavigator: true).push(
            MaterialPageRoute(
              builder: (_) => PlacementChatScreen(placementId: placementId),
            ),
          );
        }
      },
