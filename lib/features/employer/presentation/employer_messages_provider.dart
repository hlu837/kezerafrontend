...
class EmployerMessagesNotifier
  ...
  Future<void> load() async {
    ...
    try {
      ...
      // Same fix as SeekerChatThreadsNotifier.load(): don't let one job's
      // failed suggested-seekers request take down the whole list via a
      // bare Future.wait — swallow per-job failures instead.
      final perJobRows = await Future.wait([
        for (final job in employerJobs)
          jobsRepository
              .fetchSuggestedSeekers(job.id, jobTitle: job.title)
              .catchError((Object error, StackTrace stackTrace) {
            // ignore: avoid_print
            print(
              'employerMessagesProvider: failed to load suggested seekers '
              'for job ${job.id}: $error',
            );
            return <SuggestedSeeker>[];
          }),
      ]);
