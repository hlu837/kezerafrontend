...
/// Tracks which placement's row was last tapped in [EmployerMessagesScreen],
/// so that row can stay visibly highlighted — both immediately on tap (the
/// employer gets instant feedback that their selection registered before
/// the chat screen finishes pushing) and after backing out of the chat
/// (so it's obvious which conversation they were just in). `autoDispose`
/// since this is purely this screen's transient UI state, not something
/// that should survive a full app restart or leak once the screen is gone.
final selectedPlacementIdProvider = StateProvider.autoDispose<String?>(
  (ref) => null,
);
...
class _CandidateThreadRow extends ConsumerWidget {
  ...
  void _openChat(BuildContext context, WidgetRef ref) {
    // Select immediately so the row highlights the moment it's tapped,
    // not just after the chat screen has finished pushing.
    ref.read(selectedPlacementIdProvider.notifier).state = row.placementId;
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final isSelected = ref.watch(selectedPlacementIdProvider) == row.placementId;
