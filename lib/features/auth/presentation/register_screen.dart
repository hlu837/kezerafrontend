...
class RegisterScreen extends ConsumerStatefulWidget {
  ...
  /// Preselects the role tile so a deep link (e.g. the public "Agency"
  /// landing tab's CTA — see public_agency_screen.dart) can land the
  /// person straight on the right form instead of making them tap
  /// through the role picker themselves. Defaults to seeker when omitted,
  /// same as before this param existed.
  final RegisterableRole? initialRole;
...
class _RegisterScreenState extends ConsumerState<RegisterScreen> {
  ...
  // The signed business license/registration document (PDF or photo).
  // Uploaded via POST /verification/license once the account exists (that
  // endpoint requires a JWT), right before the Chapa checkout in _submit().
  ...
  void _nextStep() {
    ...
    if (_role != RegisterableRole.seeker && _licenseFile == null) {
      _showFormError(
        'Please attach your business license or company registration document.',
      );
      return;
    }
  ...
  Future<void> _submit() async {
    ...
    if (_role == RegisterableRole.seeker) {
      ...
      // Account exists now (register() just persisted a token), so the
      // authenticated /verification/license endpoint can be called. This
      // is what actually gets the document in front of admin — without
      // it, an employer/agency could pay and land in the approval queue
      // with nothing for admin to review. Best-effort: a failure here
      // shouldn't strand them mid-checkout, since /verification/resubmit
      // (reachable from the verification-status screen) lets them attach
      // it afterward.
      if (_licenseFile != null) {
        ...
        try {
          await ref
              ...
              .uploadLicense(_licenseFile!);
        } on ApiException catch (e) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  "Couldn't upload your license document (${e.message}). "
                  'You can attach it again from the verification screen.',
                ),
              ),
            );
          }
  ...
  Widget build(BuildContext context) {
    ...
    final isSubmitting =
        authState.status == AuthStatus.authenticating || _isUploadingLicense;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          icon: Icon(Icons.arrow_back_ios_new, color: AppColors.ink, size: 20),
    ...
    switch (_role) {
      ...
      case RegisterableRole.employer:
        return [
          ...
          const SizedBox(height: 18),
          _buildLicensePicker(isSubmitting),
          const SizedBox(height: 18),
        ];
      case RegisterableRole.agency:
        return [
  ...
  // ---------------------------------------------------------------------------
  // Business License Picker — attaches the document validated later by
  // POST /verification/license (PDF or a photo of the printed license).
  // ---------------------------------------------------------------------------

  Widget _buildLicensePicker(bool isSubmitting) {
    final hasFile = _licenseFile != null;
    return InkWell(
      onTap: isSubmitting ? null : _pickLicenseFile,
      borderRadius: BorderRadius.circular(12),
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(12),
          border: Border.all(
            color: hasFile ? AppColors.green : AppColors.border,
          ),
        ),
        child: Row(
          children: [
            Icon(
              hasFile ? Icons.task_outlined : Icons.upload_file_outlined,
              color: hasFile ? AppColors.green : AppColors.inkMuted,
              size: 20,
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    hasFile ? _licenseFile!.filename : 'Tap to attach (PDF, JPG or PNG)',
                    style: TextStyle(
                      color: hasFile ? AppColors.ink : AppColors.inkMuted,
                      fontSize: 14,
