...
class _VerificationCard extends ConsumerWidget {
            ...
            children: [
              if (licenseUrl != null)
                _SmallButton(
                  ...
                  // Used to just toast the raw URL as text instead of
                  // opening it, so an admin could never actually look at
                  // the uploaded TIN/license document to verify it. Same
                  // launchUrl(..., mode: externalApplication) pattern used
                  // for CVs and checkout links elsewhere in this app.
                  onTap: () => _openLicense(context, licenseUrl),
                )
              else
  ...
  Future<void> _openLicense(BuildContext context, String licenseUrl) async {
    final uri = Uri.tryParse(licenseUrl);
    bool launched = false;
    if (uri != null) {
      launched =
    ...
    if (!launched && context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Couldn't open the license document."),
        ),
      );
    }
  }
  ...
  Future<void> _reject(BuildContext context, WidgetRef ref, String userId,
    ...
    try {
      ...
      onActionComplete();
      if (context.mounted) {
        final refund = (response.data?['data']
            as Map<String, dynamic>?)?['refund'] as Map<String, dynamic>?;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(_rejectionSnackbarMessage(refund)),
              backgroundColor: const Color(0xFFE57373)),
        );
      }
    } catch (e) {
  ...
  /// Rejecting now also attempts to refund the account's latest
  /// subscription payment server-side (admin.service.js#rejectUser) —
  /// this turns that outcome into copy so the admin knows whether
  /// money actually went back or needs manual follow-up, instead of
  /// the generic "Account rejected." that gave no signal either way.
  String _rejectionSnackbarMessage(Map<String, dynamic>? refund) {
    switch (refund?['status'] as String?) {
      case 'refunded':
        final amount = refund?['amount'];
        final currency = refund?['currency'] ?? 'ETB';
        return amount != null
            ? 'Account rejected. Refunded $amount $currency.'
            : 'Account rejected. Payment refunded.';
      case 'refund_failed':
        return 'Account rejected. Refund failed — follow up with the company manually.';
      default:
