...
/// format), plus optional CV/photo attachments, experience level, and
/// preferred job categories (job type) so a desk worker can capture the
/// same profile detail a self-registered seeker would set themselves.
class WalkInRegistrationScreen extends ConsumerStatefulWidget {
  const WalkInRegistrationScreen({super.key, this.onRegistered});

  /// Called after a candidate is registered successfully — lets the
  /// parent (see `AgencyRegistrationScreen`) refresh the "Registered"
  /// tab's roster and jump the user straight to it.
...
class _WalkInRegistrationScreenState
  ...
  void _toggleCategory(String key) {
    setState(() {
      if (_preferredCategories.contains(key)) {
        _preferredCategories.remove(key);
      } else {
        _preferredCategories.add(key);
      }
    });
  }
                        ...
                        children: [
                          for (final category in kJobCategories)
                            FilterChip(
                              ...
                              selected:
                                  _preferredCategories.contains(category.key),
                              onSelected: _isSubmitting
                                  ? null
                                  : (_) => _toggleCategory(category.key),
                            ),
                        ],
                      ),
                      const SizedBox(height: 14),
