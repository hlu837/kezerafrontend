...
class MockJobs {
  ...
  static final List<Job> items = [
    Job(
      ...
      description:
          'Join our product team building the Kezera mobile app. '
  ...
  static List<Job> filtered(JobBrowseParams params) {
    return items.where((job) {
      ...
      // JS-06: "For You" — same $in-style "matches ANY" semantics as
      // jobs.service.js#browseJobs, and same precedence over the
      // single `category` filter above (checked first, but harmless
      // to also check here since JobBrowseParams never sets both).
      if (params.categories.isNotEmpty &&
          !params.categories.contains(job.category)) {
        return false;
      }
      if (params.creatorType != null &&
          params.creatorType!.isNotEmpty &&
          job.creatorType != params.creatorType) {
        return false;
      }
      if (params.experienceLevel != null &&
          params.experienceLevel!.isNotEmpty &&
