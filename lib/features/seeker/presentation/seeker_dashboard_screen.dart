...
class _BoostCardState extends ConsumerState<_BoostCard> with WidgetsBindingObserver {
  bool _isLaunching = false;

  // Set once the Chapa checkout tab has actually been opened, so we know
  // a résumé of the app is worth reacting to. Without this, coming back
  // from *any* backgrounding (a notification, switching apps) would fire
  // an unnecessary profile refetch.
  bool _awaitingReturnFromCheckout = false;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed && _awaitingReturnFromCheckout) {
      _awaitingReturnFromCheckout = false;
      // Best-effort: picks up `boostedUntil` once Chapa's /verify-callback
      // has run server-side. If the person returns before that lands, the
      // card just keeps its existing pull-to-refresh as a fallback.
      ref.read(myProfileProvider.notifier).refreshSilently();
    }
  }
  ...
  Future<void> _boost() async {
    ...
    try {
      ...
      _awaitingReturnFromCheckout = true;
      ...
      if (!launched) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Couldn't open the payment page. Please try again.")),
          );
        }
      }
    } on ApiException catch (e) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Something went wrong starting the payment. Please try again.')),
        );
      }
    } finally {
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    isBoosted ? 'Profile boosted' : "Can't find you? Boost your profile",
                    style: Theme.of(context).textTheme.titleMedium,
                  ),
                  const SizedBox(height: 4),
                  Text(
