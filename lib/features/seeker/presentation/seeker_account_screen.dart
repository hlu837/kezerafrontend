...
class SeekerAccountScreen extends ConsumerWidget {
  ...
  Future<void> _showEditProfileDialog(
    ...
    var experienceLevel = profile.experienceLevel;
    var isSaving = false;
    String? errorMessage;
                ...
                children: [
                  ...
                  DropdownButtonFormField<ExperienceLevel?>(
                    ...
                    decoration: const InputDecoration(
                      ...
                      helperText:
                          'Shown to employers/agencies searching for candidates.',
                    ),
                    items: [
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              _AvatarPicker(photoUrl: profile.photoUrl, initials: initials),
        ...
        _AccountCard(
          children: [
            ...
            _AccountRow(
              ...
              label: 'Trade Technician profile',
              onTap: () => context.push('/seeker/onboarding/technician-registration'),
            ),
            const Divider(height: 1),
            _AccountRow(
            ...
            _AccountRow(
              ...
              onTap: () => context.go('/seeker/service-requests'),
            ),
          ],
        ),
      ],
...
class _AvatarPicker extends ConsumerStatefulWidget {
  const _AvatarPicker({required this.photoUrl, required this.initials});
...
/// Tapping the avatar (or its camera badge) picks an image and uploads it
/// via the same `POST /seekers/upload` endpoint the dashboard's "Photo"
/// row already uses (see `seeker_dashboard_screen.dart`'s `_UploadRow`
/// with `fieldName: 'photo'`) — this is just a second, avatar-shaped entry
/// point onto the same `myProfileProvider.uploadFiles(photo: ...)` call.
class _AvatarPickerState extends ConsumerState<_AvatarPicker> {
  ...
  Future<void> _pickAndUpload() async {
    ...
    try {
      await ref.read(myProfileProvider.notifier).uploadFiles(
            photo: WalkInAttachment(bytes: file.bytes!, filename: file.name),
          );
    } on ApiException catch (e) {
      if (mounted) {
