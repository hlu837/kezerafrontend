...
class MyProfileNotifier extends StateNotifier<AsyncValue<Seeker>> {  MyProfileNotifier(this._repository) : super(const AsyncValue.loading());
  ...
  /// Re-fetches the profile without flipping [state] to
  /// `AsyncValue.loading()` first — used after the person returns from the
  /// Chapa checkout tab so `boostedUntil` picks up the server-confirmed
  /// value (see payment.routes.js's /verify-callback) without flashing the
  /// whole dashboard back to a loading skeleton. Failures are swallowed:
  /// this is a best-effort background refresh, not a user-initiated action,
  /// so there's nothing useful to show an error for and the last-known
  /// profile should simply stay on screen.
