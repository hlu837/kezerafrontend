...
class CvEducation extends Equatable {
  ...
  final String? endDate;
  final bool current;
  // Optional, 0.00-4.00 — not every institution grades on a GPA scale,
  // so this stays nullable rather than defaulting to 0. Used by
  // employers to rank/summarize applicants (see ApplicationSummaryScreen).
  final double? gpa;
  ...
  @override
  List<Object?> get props =>
      [school, degree, fieldOfStudy, startDate, endDate, current, gpa];
}
...
class Seeker extends Equatable {
  ...
  // — see ExperienceLevel. Null until the seeker sets one.
  final ExperienceLevel? experienceLevel;
  // Employer/agency-facing "last seen" timestamp — merged in server-side
  // from User.lastActiveAt (see utils/attachLastSeen.js). Only present
  // on employer/agency-facing responses (search, agency roster);
  // GET /seekers/me does not include it. Null if never seen since the
  // field was introduced.
  final DateTime? lastSeenAt;
  // Contact number — merged in server-side from User.phone (see
  // utils/attachLastSeen.js#attachPhone), same "only on certain
  // employer/agency-facing responses" shape as `lastSeenAt`. Unlike
  // `lastSeenAt`, only present on the agency's own roster
  // (GET /agencies/candidates) — never on the general candidate search
  // pool, since a phone number is only surfaced where the caller
  // already has a legitimate relationship with the seeker. Null
  // everywhere else, including GET /seekers/me.
  final String? phone;
...
class SearchSeekersParams {
  ...
  final String? city;
  final List<String> skills;
  // Job category key (see kJobCategories) and seniority band — the
  // "Find candidates" filter's Category/Experience dropdowns.
  ...
  Map<String, dynamic> toQuery() => {
        ...
        if (experienceLevel != null)
          'experienceLevel': experienceLevel!.wireValue,
        'page': page,
        'limit': limit,
      };

  /// [clearCategory]/[clearExperienceLevel] let the filter form reset a
  /// dropdown back to "Any" — a plain `copyWith` can't tell "leave
  /// unchanged" apart from "set back to null" since both look like a
  /// null argument.
...
/// One row of `GET /technicians/trade-categories` — a trade category plus
/// how many currently-available experts are listed under it. Backs the
/// "Electricians · 12" cards on [ExpertCategoriesScreen]; the label
/// comes from the server so it always matches the count.
class ExpertCategoryCount {
  const ExpertCategoryCount({
    required this.key,
    required this.label,
    required this.count,
  });
  ...
  factory ExpertCategoryCount.fromJson(Map<String, dynamic> json) =>
      ExpertCategoryCount(
