...
class SeekerProfileRepository {
  ...
  /// GET /seekers/me/placements — EMP-02: this seeker's own placements
  /// (one per job the matching engine has put them forward for), used to
  /// list their message threads (`/placements/:placementId/messages`).
  Future<List<MyPlacement>> fetchMyPlacements() => _guard(() async {
        final response =
            await _apiClient.dio.get<Map<String, dynamic>>('/seekers/me/placements');
        final data = response.data!['data'] as Map<String, dynamic>;
        return (data['placements'] as List<dynamic>)
            .map((json) => MyPlacement.fromJson(json as Map<String, dynamic>))
            .toList();
      });
