...
class AtsRepository {
  ...
  /// POST /placements/invite
  ///
  /// "Send Job Request" / "Chat" from the Experts map/directory (or any
  /// other candidate search): upserts a `matched`-status Placement
  /// linking [jobId] and [seekerId], returning its id — which doubles
  /// as the messaging thread key for [PlacementChatScreen]. Safe to
  /// call again for the same pair (idempotent server-side upsert), so
  /// "Chat" can call this on every open rather than tracking locally
  /// whether a placement already exists.
  Future<String> inviteCandidate({
    ...
    required String seekerId,
      ...
      _guard(() async {
        final response = await _apiClient.dio.post<Map<String, dynamic>>(
          '/placements/invite',
          data: {'jobId': jobId, 'seekerId': seekerId},
        );
        final data = response.data!['data'] as Map<String, dynamic>;
        return data['placementId'] as String;
      });
