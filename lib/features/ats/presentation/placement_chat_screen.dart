...
///
/// IMPORTANT for every call site: every role's shell (`employer_shell`,
/// `seeker_shell`, `agency_shell`) is a go_router `ShellRoute`, which
/// builds its own nested `Navigator` around the tab content so the
/// bottom nav/sidebar can persist across tab switches. A plain
/// `Navigator.push(context, ...)` from inside that content lands on
/// *that* nested Navigator, not the app's root one — so this screen
/// would render squeezed inside the shell's chrome (duplicate app bar,
/// bottom nav bar still showing, layout overflow) instead of taking
/// over the full screen. Always push this (and any other full-screen
/// page reached from inside a shell) with
...
class PlacementChatScreen extends ConsumerStatefulWidget {
  ...
  /// Optional because a couple of entry points (e.g. tapping a
  /// "new message" notification, which only carries `placementId` — see
  /// `notifications.service.js#notifyNewMessage`) don't have the other
  /// participant's name on hand without an extra round trip. Falls back
  /// to a generic "Conversation" title in the AppBar when omitted.
...
class _PlacementChatScreenState extends ConsumerState<PlacementChatScreen> {
  ...
  /// Polls the thread while it's open so a reply from the other
  /// participant shows up on its own — there's no websocket/push
  /// transport anywhere in this app (see `notifications_provider.dart`'s
  /// `_pollInterval` doc comment for the same reasoning), so this is
  /// the same "poll a cheap-ish existing endpoint" fix applied here.
  /// `getMessages` doesn't support "since this id", so each tick
  /// refetches the full thread; that's fine at this message-volume
  /// scale and it's also what `listMessages` already does server-side
  /// on every open (it re-marks the other party's messages read each
  /// time, same as before).
  ...
  /// Background counterpart to [_loadMessages]: silent (no loading
  /// spinner, no error banner — a missed tick just tries again in
  /// [_pollInterval]) and careful not to yank the scroll position out
  /// from under someone reading back through history. Skipped entirely
  /// while a send is in flight, so it can't race the optimistic append
  /// in [_handleSend].
  Future<void> _pollMessages() async {
    ...
    try {
      ...
      if (!hasNewMessages && messages.length == _messages.length) {
        // Nothing new arrived and nothing to reconcile (e.g. a
        // read-receipt flip on a message the *other* party sent isn't
        // worth a rebuild here) — skip the setState entirely.
        return;
      }
  ...
  /// Within ~80px of the bottom counts as "already caught up", so a
  /// newly-arrived message auto-scrolls into view; further up than
  /// that means the user is deliberately reading history, and a poll
  /// tick shouldn't yank them back down.
  bool _isNearBottom() {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text(
              widget.candidateName ?? 'Conversation',
              maxLines: 1,
